<template>
  <div class="container">
      ...
      <div class="row">
        <div class="col">
          ...
          >
            ...
            <option v-for="classe in uniqueClasses" :key="classe">
              {{ classe }}
            </option>
          </select>
        </div>
    ...
    <!-- Add Student Modal -->
    <StudentForm
      v-if="showModal && mode === 'add'"
      mode="add"
      :editedData="this.newStudent"
      :majors="majors"
      :years="years"
...
export default {
    ...
    uniqueClasses() {
      ...
      this.students.forEach((student) => {
        if (student.major && student.year) {
          classes.add(
            `${student.major.majorName} - ${student.year.academicYear}`
          );
        }
      });
  ...
  methods: {
    ...
    async editStudent(studentData) {
      try {
        await axios.put(
          `http://localhost:1111/api/v1/students/${this.student_id}`,
          studentData
        );
        // Refresh the students list
        const response = await axios.get(
